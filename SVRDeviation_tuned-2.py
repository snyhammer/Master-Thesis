...
OPEC = {
    ...
    "NGA":"Nigeria",      "DZA":"Algeria", "GAB":"Gabon",        "COG":"Congo",   "GNQ":"Eq. Guinea",
}
OPEC_NAME_MAP = {
    "Algeria":"DZA","Congo":"COG","Equatorial Guinea":"GNQ","Gabon":"GAB",
    "IR Iran":"IRN","Iraq":"IRQ","Kuwait":"KWT","Nigeria":"NGA","Saudi Arabia":"SAU","United Arab Emirates":"ARE",
    "Venezuela":"VEN",
}
REF_COUNTRY = "SAU"
...
print(f"  CV mean    MAE:  {cv_mae:.2f} pp")
...
for _, r in fold_df.iterrows():
    print(f"  {r['fold']:>4}  {r['test_from']}–{r['test_to']:>8}  "
          f"{r['r2_test']:>+8.4f}  {r['rmse_test']:>8.2f}pp  "
          f"{r['best_C']:>7}")
...
plt.tight_layout()
plt.savefig(os.path.join(DATA_DIR, 'fig_svr_dev_tuned.png'), dpi=180, bbox_inches='tight')


# ── Figure: R², RMSE, MAE bar chart (in-sample vs OOS) ──────────────────────
import matplotlib.gridspec as _gs

_metrics = ["R²", "RMSE (pp)", "MAE (pp)"]
_is_vals = [is_r2, is_rmse, is_mae]
_oos_vals= [oos_r2, oos_rmse, oos_mae]

fig3, axes3 = plt.subplots(1, 3, figsize=(12, 4))
_colours = {"In-sample": "#003153", "OOS (pooled)": "#c0392b"}

for ax3, metric, is_v, oos_v in zip(axes3, _metrics, _is_vals, _oos_vals):
    bars = ax3.bar(
        list(_colours.keys()), [is_v, oos_v],
        color=list(_colours.values()), width=0.5, alpha=0.88,
    )
    ...
    ax3.set_title(metric, fontsize=13, pad=10)
    ax3.spines["top"].set_visible(False)
    ax3.spines["right"].set_visible(False)
    ax3.tick_params(axis="x", labelsize=10)
    ax3.grid(axis="y", alpha=0.2, linewidth=0.6)
    if metric != "R²":
        ax3.set_ylim(bottom=0)

plt.tight_layout(pad=2.0)
plt.savefig(os.path.join(DATA_DIR, "fig_svr_dev_metrics_tuned.png"), dpi=180, bbox_inches="tight")
print("Saved: fig_svr_dev_metrics.png")

# ── 9. Figure: permutation importance ────────────────────────────────────────

# Friendly display labels
LABEL_MAP = {
    "ODS_l6":    "Oil demand shock (ODS)",
    "ORS_l6":    "Oil risk shock (ORS)",
    "GPR_l12":   "GPR shock",
    "ln_vix_12m":"VIX 12m average",
    "oil_gdp":   "Oil rents % GDP",
    "inst":      "Institutional quality",
    "dev_pct_l1":"Lagged quota deviation (t−1)",
    "C_IRN":     "Iran (FE)",
    "C_IRQ":     "Iraq (FE)",
    "C_KWT":     "Kuwait (FE)",
    "C_ARE":     "UAE (FE)",
...
all_labels = [LABEL_MAP.get(f, f) for f in ALL_F]

# Sort ascending so most important appears at the top
sort_order     = np.argsort(imp_mean)
sorted_labels  = [all_labels[i] for i in sort_order]
sorted_mean    = imp_mean[sort_order]
sorted_std     = imp_std[sort_order]

# Single colour: blue for positive, red for negative
bar_colours = ["#4575b4" if v >= 0 else "#c0392b" for v in sorted_mean]

fig2, ax2 = plt.subplots(figsize=(9, 7))
y_pos = np.arange(len(sorted_mean))
...
ax2.axvline(0, color="#888888", linewidth=0.9, linestyle="--", alpha=0.7)
ax2.set_yticks(y_pos)
ax2.set_yticklabels(sorted_labels, fontsize=10)
ax2.set_xlabel("R²", fontsize=11, labelpad=8)
...
plt.tight_layout()
plt.savefig(os.path.join(DATA_DIR, "fig_svr_dev_importance_tuned.png"), dpi=180, bbox_inches="tight")
print("Saved: fig_svr_dev_importance_tuned.png")
...
pd.DataFrame(rows).to_csv(os.path.join(DATA_DIR, "results_svr_dev_tuned.csv"), index=False)
print("Saved: results_svr_dev_tuned.csv")
