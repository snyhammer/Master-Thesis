"""
OPEC Oil Production — SVR with RBF Kernel
==========================================
Dependent variable:
    yoy_prod = ln(prod_t) − ln(prod_{t-12})
    Year-on-year log production change. Not standardised.
    Converts to approximate % change via (e^β − 1) × 100.
...
Features (standardised to mean=0, std=1 on each training fold):
    ODS_l6, ORS_l6, GPR_l12, ln_vix_12m, oil_gdp, inst, yoy_lag1
    + country dummies (SAU = reference, omitted)

Validation — Rolling Time Series Cross-Validation:
    Balanced panel: 12 countries × 377 months = 4,512 obs.
    Fixed training window: 864 rows = 72 months × 12 countries (6 years).
    Step: 144 rows = 12 months × 12 countries (1 year).
    25 folds total, covering 2000–2025.
...
Outputs:
    results_svr_prod.csv           — fold metrics + importance
    fig_svr_prod.png               — shock contributions over time
    fig_svr_prod_importance.png    — permutation feature importance
...
#   Balanced panel: 12 countries × 377 months = 4,512 obs
#   Window = 864 rows = 72 months × 12 countries (6 years)
#   Step   = 144 rows = 12 months × 12 countries (1 year)
#   → 25 folds, covering 2000–2025
WINDOW_ROWS = 864
STEP_ROWS   = 144
...
def load_inst():
    ...
    nm  = {
        ...
        "Iran":"IRN",    "Iraq":"IRQ",               "Kuwait":"KWT",
        "Libya":"LBY",   "Nigeria":"NGA",             "Saudi Arabia":"SAU",
        "United Arab Emirates":"ARE", "Venezuela":"VEN", "Congo":"COG",
    }
...
def build_shocks(brent, vix, gpr):
    ...
    ORS ⊥ ODS by construction. All standardised to N(0,1).
    """
...
panel = prod.merge(shocks, on="date", how="inner")
...
panel["yoy_prod"]   = panel.groupby("iso3")["ln_prod"].transform(
...
panel["yoy_lag1"]   = panel.groupby("iso3")["yoy_prod"].shift(1)

for iso3 in OPEC:
    if iso3 != REF_COUNTRY:
        panel[f"C_{iso3}"] = (panel["iso3"] == iso3).astype(float)

KEY = ["yoy_prod","ODS_l6","ORS_l6","GPR_l12",
       "ln_vix_12m","oil_gdp","inst","yoy_lag1"]
p = panel.dropna(subset=KEY).copy()

SHOCK_F  = ["ODS_l6","ORS_l6","GPR_l12"]
CTRL_F   = ["ln_vix_12m","oil_gdp","inst","yoy_lag1"]
...
y     = p["yoy_prod"].values.astype(float)
dates = p["date"].values
...
print(f"DV (yoy_prod): mean={y.mean():+.4f}  std={y.std():.4f}  "
      f"range [{y.min():.3f}, {y.max():.3f}]")
print(f"\nRolling CV: window={WINDOW_ROWS} rows (72 months × 12 countries = 6 yrs), "
      f"step={STEP_ROWS} rows (12 months × 12 countries = 1 yr)")

# ── 4. Rolling Time Series Cross-Validation ───────────────────────────────────

n = len(p)
fold_starts = list(range(0, n - WINDOW_ROWS - STEP_ROWS + 1, STEP_ROWS))
...
last_start = (len(fold_results) - 1) * STEP_ROWS
last_tr    = np.arange(last_start, last_start + WINDOW_ROWS)
last_te    = np.arange(last_start + WINDOW_ROWS,
                        min(last_start + WINDOW_ROWS + STEP_ROWS, n))
...
print(f"\n\n{'='*65}")
print("RESULTS SUMMARY — SVR RBF, YoY Production as DV")
print(f"{'='*65}")
print(f"DV:      yoy_prod = ln(prod_t) − ln(prod_{{t-12}})")
...
print(f"  CV mean    RMSE: {cv_rmse:.4f} log pts")
print(f"  OOS pooled R²:   {oos_r2:+.4f}")
print(f"  OOS pooled RMSE: {oos_rmse:.4f} log pts")
print(f"\nFold-level:")
print(f"  {'Fold':>4}  {'Test period':>22}  {'R²':>8}  {'RMSE':>8}  {'Best C':>7}")
print("  " + "─"*55)
for _, r in fold_df.iterrows():
    print(f"  {r['fold']:>4}  {r['test_from']}–{r['test_to']:>8}  "
          f"{r['r2_test']:>+8.4f}  {r['rmse_test']:>8.4f}   "
          f"{r['best_C']:>7}")

# ── 8. Figure: shock contributions to YoY production over time ───────────────
#
...
ax.set_ylabel("Contribution to YoY production change (log pts)", fontsize=12, labelpad=8)
...
plt.tight_layout()
plt.savefig("fig_svr_prod.png", dpi=180, bbox_inches="tight")
print("\n\nSaved: fig_svr_prod.png")
...
plt.tight_layout()
plt.savefig("fig_svr_prod_importance.png", dpi=180, bbox_inches="tight")
print("Saved: fig_svr_prod_importance.png")
...
for _, r in fold_df.iterrows():
    rows.append({
        "type":       "cv_fold",
        "fold":       r["fold"],
        "period":     f"{r['test_from']}–{r['test_to']}",
        "n_train":    r["n_train"],
        "n_test":     r["n_test"],
        "r2":         r["r2_test"],
        "rmse":       r["rmse_test"],
...
for label, rv, rmv in [
    ("in_sample",  is_r2,   is_rmse),
    ("cv_mean",    cv_r2,   cv_rmse),
    ("oos_pooled", oos_r2,  oos_rmse),
]:
    rows.append({"type": label, "r2": rv, "rmse": rmv})

for fi, feat in enumerate(ALL_F):
    group = ("shock"   if feat in SHOCK_F else
             "control" if feat in CTRL_F  else "dummy")
    rows.append({
        "type":            "importance",
        "feature":         feat,
        "group":           group,
        "importance_mean": imp_mean[fi],
        "importance_std":  imp_std[fi],
    })

pd.DataFrame(rows).to_csv("results_svr_prod.csv", index=False)
print("Saved: results_svr_prod.csv")
